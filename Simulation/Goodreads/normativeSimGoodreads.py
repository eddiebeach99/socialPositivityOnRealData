import numpy as np
import pandas as pd
import modelSimGoodreads as ms
import itertools
import matplotlib.pyplot as plt
import random

data_poor = pd.read_csv("C:/Users/edwar/SocialPositivityData/GoodreadsData/goodreads_poor.csv")
data_rich = pd.read_csv("C:/Users/edwar/SocialPositivityData/GoodreadsData/goodreads_rich.csv")

unique_products_rich = data_rich["book_id"].unique()
df_filtered_rich = data_rich[data_rich["book_id"].isin(unique_products_rich)]
unique_products_poor = data_poor["book_id"].unique()
df_filtered_poor = data_poor[data_poor["book_id"].isin(unique_products_poor)]

# Find 50 bandits that have at least 0.2 difference in average review recommendation
def filter_bandits(data):
    grouped = data.groupby('book_id')
    item_recommendations = {book_id: list(zip(group['rating'], group['user_id'])) for book_id, group in grouped}

    ratings = {book_id: [rating for rating, user_id in recs] for book_id, recs in item_recommendations.items()}
    user_ids = {book_id: [user_id for rating, user_id in recs] for book_id, recs in item_recommendations.items()}
    ...
    bandits_amazon_poor.to_csv("Simulation/Goodreads/bandits_goodreads_rich.csv", index=False)
...
def simulation(file):
    ...
    result.to_csv("C:/Users/edwar/SocialPositivityData/Goodreads/simulation_goodreads_poor.csv", index=False)
...
simulation("Simulation/Goodreads/bandits_goodreads_poor.csv")
