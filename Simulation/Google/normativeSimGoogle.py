import numpy as np
import pandas as pd
import modelSimGoogle as ms
import itertools
import matplotlib.pyplot as plt
import random

data_poor = pd.read_csv("C:/Users/edwar/SocialPositivityData/GoogleData/google_poor.csv")
data_rich = pd.read_csv("C:/Users/edwar/SocialPositivityData/GoogleData/google_rich.csv")

unique_products_rich = data_rich["gmap_id"].unique()
df_filtered_rich = data_rich[data_rich["gmap_id"].isin(unique_products_rich)]
unique_products_poor = data_poor["gmap_id"].unique()
df_filtered_poor = data_poor[data_poor["gmap_id"].isin(unique_products_poor)]

# Find 50 bandits that have at least 0.2 difference in average review recommendation
def filter_bandits(data):
    grouped = data.groupby('gmap_id')
    item_recommendations = {gmap_id: list(zip(group['rating'], group['user_id'])) for gmap_id, group in grouped}

    ratings = {gmap_id: [rating for rating, user_id in recs] for gmap_id, recs in item_recommendations.items()}
    user_ids = {gmap_id: [user_id for rating, user_id in recs] for gmap_id, recs in item_recommendations.items()}
    ...
    bandits_amazon_poor.to_csv("Simulation/Google/bandits_google_poor.csv", index=False)
...
def simulation(file):
    ...
    result.to_csv("C:/Users/edwar/SocialPositivityData/Google/simulation_google_rich.csv", index=False)
...
simulation("Simulation/Google/bandits_google_rich.csv")
