import numpy as np
import pandas as pd
import modelSimYelp as ms
import itertools
import matplotlib.pyplot as plt
import random

data_poor = pd.read_csv("C:/Users/edwar/SocialPositivityData/YelpData/yelp_poor.csv")
data_rich = pd.read_csv("C:/Users/edwar/SocialPositivityData/YelpData/yelp_rich.csv")

unique_products_rich = data_rich["business_id"].unique()
df_filtered_rich = data_rich[data_rich["business_id"].isin(unique_products_rich)]
unique_products_poor = data_poor["business_id"].unique()
df_filtered_poor = data_poor[data_poor["business_id"].isin(unique_products_poor)]
...
def filter_bandits(data):
    ...
    item_recommendations = {business_id: list(zip(group['stars'], group['user_id'])) for business_id, group in grouped}

    starss = {business_id: [stars for stars, user_id in recs] for business_id, recs in item_recommendations.items()}
    user_ids = {business_id: [user_id for stars, user_id in recs] for business_id, recs in item_recommendations.items()}
    ...
    for (item_id_a, recs_a), (item_id_b, recs_b) in (itertools.combinations(starss.items(), 2)):
        i += 1
        ...
        if np.abs(np.mean(recs_a) - np.mean(recs_b)) > 0.2:
            ...
            # Check if there is a unique user who gave one item a negative review and the other a positive review
            ...
            for user_id in set(user_ids_a).intersection(set(user_ids_b)):
                ...
                if (review_a == 1 and review_b == 0) or (review_a == 0 and review_b == 1):
                    ...
                    filtered_recs_a = [stars for idx, stars in enumerate(recs_a) if user_ids_a[idx] != conflicting_user]
                    filtered_recs_b = [stars for idx, stars in enumerate(recs_b) if user_ids_b[idx] != conflicting_user]
    ...
    bandits_amazon_poor.to_csv("Simulation/Yelp/bandits_yelp_poor.csv", index=False)
...
def simulation(file):
    ...
    result.to_csv("C:/Users/edwar/SocialPositivityData/Yelp/simulation_yelp_rich.csv", index=False)


# Find out which bandits to use and then use the simulation function on them, don't forget to change the file names! 

# filter_bandits(df_filtered_poor)
simulation("Simulation/Yelp/bandits_yelp_rich.csv")
